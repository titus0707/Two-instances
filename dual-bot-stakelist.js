...
class TradingInstance {
    ...
    resolveTrade(digit, result) {
        ...
        if (isWin) {
            ...
            if (this.name === 'B') {
                // Instance B WIN = RESET to Round 1
            ...
            if (this.name === 'A') {
                // Instance A LOSS = RESET to Round 1
                this.parentBot.instanceA.roundIndex = 0;
                this.parentBot.instanceB.roundIndex = 0;
                this.parentBot.log(`\x1b[91m[A LOSS] Both instances reset to Round 1\x1b[0m`);
            } else {
                // Instance B LOSS = PROGRESS to next round
                if (this.roundIndex < STAKE_A.length - 1) {
                    this.parentBot.instanceA.roundIndex++;
                    this.parentBot.instanceB.roundIndex++;
                    this.parentBot.log(`\x1b[91m[B LOSS] Both instances moved to Round ${this.parentBot.instanceA.roundIndex + 1}\x1b[0m`);
                }
            }
...
class StreakReversalBot {
    constructor() {
        ...
        this.totalProfit = 0;
        this.profitTarget = 10; // $10 profit target
    ...
    handleBuyResponse(response) {
        if (response.error) {
            ...
            if (response.error.code === 'InvalidOfferings') {
                this.log('\x1b[91mInvalid symbol/contract - switching to demo account\x1b[0m');
                this.symbol = "1HZ10V";
            }
        } else {
    ...
    processTick(tick) {
        ...
        const resultA = { won: false, lost: false };
        const resultB = { won: false, lost: false };

        this.resolveSingleTrade(this.instanceA, digit, resultA);
        this.resolveSingleTrade(this.instanceB, digit, resultB);

        const price = parseFloat(tick.quote).toFixed(2);
        this.log(`Tick ${this.tickCount} | Price: ${price} | Digit: ${digit} (${isOdd ? 'ODD' : 'EVEN'})`);

        const outcomeSummary = [];
        if (resultA.won) outcomeSummary.push('[A: WIN]');
        if (resultA.lost) outcomeSummary.push('[A: LOSS]');
        if (resultB.won) outcomeSummary.push('[B: WIN]');
        if (resultB.lost) outcomeSummary.push('[B: LOSS]');

        this.log(`📊 Outcome: ${outcomeSummary.join(' ')}`);

        this.processInstance(this.instanceA, isOdd, isEven, digit);
        this.processInstance(this.instanceB, isOdd, isEven, digit);
    }

    resolveSingleTrade(instance, digit, result) {
        instance.resolveTrade(digit, result);
    }
    ...
    processInstance(instance, isOdd, isEven, digit) {
        ...
        if (isOdd) {
            ...
            if (!instance.oddStreak.traded) {
                ...
                if (instance.oddStreak.count === 1 && !instance.oddStreak.traded) {
            ...
            if (!instance.evenStreak.traded) {
                ...
                if (instance.evenStreak.count === 1 && !instance.evenStreak.traded) {
